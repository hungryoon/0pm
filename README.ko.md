...
`/0pm:sync`가 새 프로젝트를 감지하면 `0pm.config.yaml`, `docs/` 구조, 템플릿, `.gitignore`를 자동 생성합니다. 실제 코드 레포는 `repos/` 아래에 위치합니다 (gitignore됨).
...
`/0pm:sync`가 제일 중요합니다. 코드베이스를 스캔해서 구조화된 문서를 만들고, Claude가 이후 모든 세션에서 참조할 수 있게 합니다. 이게 없으면 Claude는 추측하고, 이게 있으면 뭐가 있고 뭐가 중요한지 압니다.
